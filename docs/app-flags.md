...
ב-2026-09; זהו המקום היחיד שבו עורכים אותם. Bagel ממשיך להתעדכן, אבל
אוטומטית, לאפליקציות הישנות בלבד.
...
- אם השיקוף נכשל, המסך אומר זאת במפורש ("נשמר ל-Firestore, אבל Bagel לא
  עודכן") ומציע "שיקוף חוזר". לא לעזוב את המסך במצב הזה: האפליקציה החדשה
  והישנות מפוצלות.
- **Bagel לא נערך ידנית יותר.** שינוי ישיר בקונסולת Bagel לא מגיע ל-Firestore,
  והאפליקציה החדשה לא תראה אותו.
...
3. **טוקן Bagel בשרת.** הפונקציה משתמשת ב-`BAGEL_TOKEN` (סטייג') ו-`PROD_BAGEL_TOKEN`
   (פרוד) מסביבת Vercel, כמו update-time. הטוקן חייב הרשאת עדכון על
   `clearTime`, `appPreferences` ו-`minAppVersion`. ב-dev מקומי מספיק
   `VITE_BAGEL_TOKEN` ב-`.env.local`.

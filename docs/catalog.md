# קטלוג תוספות (catalog)

קטלוג התוספות (Mods) הנמכרות באפליקציה (תרגום, פירוש, הכנה לתפילה, תפילה מכוונת).
הקטלוג עבר מ-BagelDB (`enhancments/items`) ל-Firestore ב-2026-09; זו הייתה
הקריאה האחרונה של האפליקציה מ-Bagel. העריכה נעשית במסך "קטלוג תוספות"
(`src/views/CatalogView.tsx`), עם מתג Stage/פרוד כמו במסך "הגדרות
אפליקציה": פרוד דורש סיסמה, ושמירה לפרוד מאחורי אישור. (מסך קולקציה רגיל
של Firecms לא מתאים כאן: הוא עובד רק מול החיבור הראשי של ה-CMS, שהוא
סטייג' גם ב-Vercel.)
...
- **במסך**: רשימה לפי סדר תצוגה, עריכה של כל שדה, הוספת תוספת חדשה
  (storeId נעול אחרי היצירה) ומחיקה מאחורי אישור כפול. שדה שרוקנו לא נכתב
  למסמך, כדי שהאפליקציה תקרא "חסר". השדות ההיסטוריים של Bagel נשמרים כפי
  שהם.
- **סטייג' ופרוד נפרדים.** הזרימה המומלצת: לערוך ולבדוק בסטייג' (באפליקציית
  סטייג'), ואז "העתקה לפרוד" בשורת התוספת ברשימת הסטייג': מעתיק את המסמך
  כפי שהוא לפרוד ודורס מסמך קיים עם אותו storeId, אחרי סיסמת פרוד ואישור.
  בכוונה אין העתקה של כל הקטלוג בבת אחת.
...
3. לפתוח את המסך "קטלוג תוספות", לבחור את הסביבה, ולוודא שעשרת המוצרים מופיעים לפי הסדר.

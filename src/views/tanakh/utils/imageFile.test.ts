...
import { formatBytes, MAX_IMAGE_DIM, slugifyFileName, targetSize, uniqueFileName } from "./imageFile";
import { isMediaPath, mediaPath } from "../services/mediaService";
...
describe("targetSize", () => {
    ...
    it("ממדים לא תקינים מחזירים אפס", () => {
...
describe("שמות קבצים", () => {
    ...
    it("שני קבצים באותו שם בזמנים שונים אינם מתנגשים", () => {
        expect(uniqueFileName("a.jpg", "webp", 1)).not.toBe(uniqueFileName("a.jpg", "webp", 2));
    });
});

describe("נתיבי מדיה", () => {
    it("mediaPath בונה נתיב תחת media/", () => {
        expect(mediaPath("e0203", "abc.webp")).toBe("media/e0203/abc.webp");
    });

    it("isMediaPath מבדיל בין קובץ באחסון למזהה תמונה אפויה", () => {
        expect(isMediaPath("media/e0203/abc.webp")).toBe(true);
        expect(isMediaPath("e0203_1_1")).toBe(false);
        expect(isMediaPath("published/content.json")).toBe(false);
    });
});

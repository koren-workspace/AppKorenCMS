 ...
 *
 * כישלון בהמרה לעולם אינו חוסם העלאה: הקובץ המקורי עולה כמות שהוא.
 */
 ...
 * או עם רווחים בלבד הופך למחרוזת ריקה ואז מוחלף ב-"image".
 */
...
export interface PreparedImage {
    ...
    sourceBytes: number;
    /** true = הקובץ המקורי עולה כמות שהוא (הקטנה לא אפשרית או לא נדרשה) */
    original: boolean;
}
...
async function loadBitmap(file: File): Promise<ImageBitmap | HTMLImageElement> {
    ...
    try {
        ...
        // הכתובת משוחררת רק אחרי שהציור הסתיים
...
export async function prepareImage(file: File, now: number = Date.now()): Promise<PreparedImage> {
    ...
    // GIF מונפש מאבד את ההנפשה בהמרה – עולה כמות שהוא
    ...
    try {
        ...
        // המרה שיצאה גדולה מהמקור (תמונה קטנה שכבר דחוסה) – עדיף המקור
